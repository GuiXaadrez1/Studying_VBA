...
Const nameSheetThow As String = "Consta_SN_ServicosTomados"

''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
' Configuracao Para formatar Itens das NF-es Recebidas - Aut   '
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

Const nameSheetOne As String = "Servicos Tomados"
    ...
    On Error GoTo Error
        ...
        Set FormatarPlanOne = Worksheets(nameSheet)
            ...
            If FormatarPlanOne.Cells(1, 1) <> "" Then
                ...
                If FormatarPlanOne.Cells(indexCellCacebalho, "E").Value = "Consta Simples Nacional" Then
                   
                    MsgBox "A Planilha ja esta no formato correto!"
                    Exit Function
                End If
                
                ' --- Caso contrario, insere todas as colunas e cabecalhos ---
                FormatarPlanOne.Columns("E").Insert
                FormatarPlanOne.Cells(indexCellCacebalho, "E").Value = "Consta Simples Nacional"
...
Private Function CruzarCnpjsWithCOllection(ByVal nameSheet As String, Optional sheetNameToCompare As String = nameSheetOne) As Variant
    ...
    lastRowToSheetCompare = SheetCompare.Cells(SheetCompare.rows.Count, "A").End(xlUp).Row
    ...
    RangeToMatriz = SheetCompare.Range(SheetCompare.Cells(4, 4), SheetCompare.Cells(lastRowToSheetCompare, 5)).Value
...
Private Function FormatCnpjAndCpfServicosTomados( _
    ...
    ' --- Loop ate encontrar celula vazia ---
...
Sub FormatarPlanilhaCruzarCnpjsAbaServicosTomados()
    ...
    Call FormatCnpjAndCpfServicosTomados
